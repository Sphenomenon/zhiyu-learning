...
export function checkProject(project, config) {
  ...
  if (production.env_vars?.AUTH_MODE?.value && production.env_vars.AUTH_MODE.value !== 'disabled') throw new Error('Production authentication differs from this release; review it before deploying.')
}
...
async function deploy() {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    try {
      ...
      if (health.ok && (await health.json()).ok && site.ok && (await site.json()).content && admin.status === 401) { healthy = true; break }
  ...
  console.log(`Production verified: ${url}. Public content is available; production sign-in remains disabled until a real identity provider is configured.`)
}
