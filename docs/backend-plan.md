...
状态：准备阶段，尚未新增远端数据库、短信账号或支付渠道。登录和经营主体需要先确定。
...
可以保留现有项目和地址，今后通过 GitHub Actions 构建，再由 Wrangler 上传。当前新增的工作流只做构建，不部署。启用自动部署时，应在 GitHub Secrets 保存专用最小权限令牌；不把本机 Wrangler OAuth 凭证复制进仓库。
